...
import { COMPANY_INFO, numberToWords } from "@/lib/billing-types";
...
const sampleInvoice = {
    ...
    customer: {
        name: "Rajesh Kumar",
        address: "123, Ashok Rajpath",
        city: "Patna, Bihar 800004",
        phone: "+91 98765 43210",
        email: "rajesh.kumar@gmail.com",
    },
    items: [
        { name: "Loom Solar 440W Mono PERC Panel", hsn: "85414011", qty: 6, rate: 18500, amount: 111000 },
        { name: "Growatt 5kW On-Grid Inverter", hsn: "85044090", qty: 1, rate: 45000, amount: 45000 },
        { name: "Panel Mounting Structure (GI)", hsn: "76109090", qty: 1, rate: 12000, amount: 12000 },
        { name: "Installation & Wiring Charges", hsn: "998314", qty: 1, rate: 15000, amount: 15000 },
        { name: "DC Cable, MC4, ACDB/DCDB Kit", hsn: "85444999", qty: 1, rate: 8000, amount: 8000 },
    ],
    subtotal: 191000,
    cgst: 17190,
    sgst: 17190,
...
export default function InvoicePreview() {
    ...
    useEffect(() => {
        const upiUrl = `upi://pay?pa=${COMPANY_INFO.upiId}&pn=${encodeURIComponent(COMPANY_INFO.name)}&am=${sampleInvoice.total}&cu=INR&tn=Invoice%20${sampleInvoice.number}`;
    ...
    const handleDownloadPDF = async () => {
        ...
        pdf.save(`Invoice-${sampleInvoice.number}.pdf`);
    };
        ...
        <div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 no-print">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl md:text-2xl font-bold text-white">Invoice Preview</h1>
                        <p className="text-gray-400 text-sm">This is how your invoices will look</p>
                    </div>
                </div>
                <div className="flex gap-2">
                    <button onClick={handleDownloadPDF} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-xl text-sm font-medium hover:bg-green-500 transition-colors">
                        <Download className="w-4 h-4" /> Download PDF
                    </button>
                    <button onClick={() => window.print()} className="flex items-center gap-2 px-4 py-2 bg-gray-800 text-gray-300 rounded-xl text-sm font-medium hover:bg-gray-700 transition-colors">
                ...
                <div ref={invoiceRef} className="bg-white rounded-2xl shadow-2xl overflow-hidden print:shadow-none" id="invoice-pdf">
                        ...
                        <div className="flex flex-col md:flex-row justify-between gap-6">
                            ...
                            <div className="text-left md:text-right">
                                <div className="inline-block bg-gray-900 text-white px-5 py-2.5 rounded-xl">
                                    ...
                                    <p className="text-lg font-bold">#{sampleInvoice.number}</p>
                                </div>
                                <div className="mt-3 space-y-1 text-xs text-gray-500">
                                    <p><span className="font-semibold text-gray-700">Date:</span> {sampleInvoice.date}</p>
                                    <p><span className="font-semibold text-gray-700">Due:</span> {sampleInvoice.dueDate}</p>
                                    <p><span className="inline-block px-2 py-0.5 rounded-full text-xs font-semibold mt-1 bg-yellow-100 text-yellow-700">● UNPAID</span></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="px-6 md:px-10 py-6">
                        <div className="bg-gray-50 rounded-xl p-5 border border-gray-100">
                            ...
                            <h3 className="text-base font-bold text-gray-900">{sampleInvoice.customer.name}</h3>
                            <div className="text-xs text-gray-500 mt-1 space-y-0.5">
                                <p>{sampleInvoice.customer.address}</p>
                                <p>{sampleInvoice.customer.city}</p>
                                <p>📞 {sampleInvoice.customer.phone} · ✉️ {sampleInvoice.customer.email}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="px-6 md:px-10 pb-6">
                        ...
                        <div className="flex justify-end mt-4">
                            <div className="w-full md:w-80 space-y-2 text-sm">
                                <div className="flex justify-between text-gray-600"><span>Subtotal</span><span className="font-medium">{formatCurrency(sampleInvoice.subtotal)}</span></div>
                                <div className="flex justify-between text-gray-600"><span>CGST (9%)</span><span className="font-medium">{formatCurrency(sampleInvoice.cgst)}</span></div>
                                <div className="flex justify-between text-gray-600"><span>SGST (9%)</span><span className="font-medium">{formatCurrency(sampleInvoice.sgst)}</span></div>
                                <div className="border-t-2 border-gray-900 pt-2">
                                    <div className="flex justify-between">
                                        <span className="text-base font-bold text-gray-900">Total Amount</span>
                                        <span className="text-base font-bold text-gray-900">{formatCurrency(sampleInvoice.total)}</span>
                                    </div>
                                </div>
                                <div className="bg-gradient-to-r from-green-500 to-emerald-500 rounded-xl p-3 mt-2">
                                    <div className="flex justify-between text-white">
                                        ...
                                        <span className="font-extrabold text-lg">{formatCurrency(sampleInvoice.total)}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="px-6 md:px-10 pb-4">
                        <div className="bg-green-50 border border-green-200 rounded-xl p-4">
                            <p className="text-xs text-green-700">
                                <span className="font-bold">Amount in Words:</span> {numberToWords(sampleInvoice.total)}
                            </p>
                        </div>
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="flex flex-col items-center bg-gray-50 rounded-xl p-5 border border-gray-100">
                                ...
                                {qrDataUrl ? (
                                    ...
                                    <div className="w-36 h-36 bg-gray-200 rounded-xl animate-pulse" />
                                )}
                                <p className="text-xs text-gray-500 mt-2 font-mono">{COMPANY_INFO.upiId}</p>
                                <p className="text-xs text-green-600 font-bold mt-1">Pay {formatCurrency(sampleInvoice.total)}</p>
                            </div>
